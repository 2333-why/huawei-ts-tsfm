...
class my_Layernorm(nn.Module):
    """Layer normalization used by Autoformer's seasonal branch."""
...
class moving_avg(nn.Module):
    """Moving average block used to split trend and seasonal components."""
...
class series_decomp_multi(nn.Module):
    """Average several series decompositions."""
    ...
    def forward(self, x):
        parts = [func(x) for func in self.series_decomp]
        seasonal = sum(part[0] for part in parts) / len(parts)
        trend = sum(part[1] for part in parts) / len(parts)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, cross, x_mask=None, cross_mask=None):
        ...
        residual_trend = self.projection(
            (trend1 + trend2 + trend3).permute(0, 2, 1)).transpose(1, 2)
