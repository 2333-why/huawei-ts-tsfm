...
class FourierFilter(nn.Module):
    ...
    def __init__(self, mask_spectrum):
        ...
        mask_spectrum = torch.as_tensor(mask_spectrum, dtype=torch.long)
        if mask_spectrum.ndim != 1 or mask_spectrum.numel() == 0:
            raise ValueError("mask_spectrum must be a non-empty one-dimensional tensor")
        self.register_buffer("mask_spectrum", mask_spectrum)
...
class KPLayer(nn.Module):
    ...
    def one_step_forward(self, z, return_rec=False, return_K=False):
        B, icudat_len, E = z.shape
        assert icudat_len > 1, 'snapshots number should be larger than 1'
        x, y = z[:, :-1], z[:, 1:]
...
class KPLayerApprox(nn.Module):
    ...
    def forward(self, z, pred_len=1):
        ...
        if pred_len <= icudat_len:
            ...
            self.K_step = torch.linalg.matrix_power(self.K, icudat_len)
            if torch.isnan(self.K_step).any():
                print('Encounter multistep K with nan, replace it by identity matrix')
                self.K_step = torch.eye(self.K_step.shape[1]).to(self.K_step.device).unsqueeze(0).repeat(B, 1, 1)
            temp_z_pred, all_pred = z, []
            for _ in range(math.ceil(pred_len / icudat_len)):
...
class TimeVarKP(nn.Module):
    ...
    def __init__(self,
        ...
        self.freq = math.ceil(self.icudat_len / self.seg_len)  # segment number of icudat
        self.step = math.ceil(self.pred_len / self.seg_len)   # segment number of output
        self.padding_len = self.seg_len * self.freq - self.icudat_len
    ...
    def forward(self, x):
        ...
        x_rec = x_rec.reshape(B, -1, self.enc_in)[:, :self.icudat_len, :]  # B L C
...
class Model(nn.Module):
    ...
    def __init__(self, configs, dynamic_dim=128, hidden_dim=64, hidden_layers=2, num_blocks=3, multistep=False):
        ...
        self.icudat_len = configs.seq_len
        self.pred_len = configs.pred_len

        # Keep the segment width independent from the requested horizon.  The
        # factory supplies a valid segment length for the input window; using
        # ``pred_len`` here makes short histories collapse to a single
        # snapshot and also changes the encoder feature width as the horizon
        # changes.
        self.seg_len = int(configs.seg_len)
        if self.seg_len < 1:
            raise ValueError("Koopa requires a positive configs.seg_len")
        ...
        if mask_spectrum is None:
            raise ValueError("Koopa requires a non-empty configs.mask_spectrum")
        mask_spectrum = torch.as_tensor(mask_spectrum, dtype=torch.long)
        if mask_spectrum.ndim != 1 or mask_spectrum.numel() == 0:
            raise ValueError("Koopa requires a non-empty configs.mask_spectrum")
        max_frequency = self.icudat_len // 2 + 1
        if torch.any(mask_spectrum < 0) or torch.any(mask_spectrum >= max_frequency):
            raise ValueError(
                "configs.mask_spectrum contains an index outside the input spectrum"
            )
        ...
        self.time_inv_encoder = MLP(f_in=self.icudat_len, f_out=self.dynamic_dim, activation='relu',
        ...
        self.time_inv_kps = self.time_var_kps = nn.ModuleList([
                                TimeInvKP(icudat_len=self.icudat_len,
    ...
    def forecast(self, x_enc):
        ...
        for i in range(self.num_blocks):
            time_var_icudat, time_inv_icudat = self.disentanglement(residual)
            time_inv_output = self.time_inv_kps[i](time_inv_icudat)
            time_var_backcast, time_var_output = self.time_var_kps[i](time_var_icudat)
