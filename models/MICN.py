...
from layers.tslib.Autoformer_EncDec import series_decomp, series_decomp_multi
...
class MIC(nn.Module):
    ...
    def conv_trans_conv(self, icudat, conv1d, conv1d_trans, isometric):
        batch, seq_len, channel = icudat.shape
        x = icudat.permute(0, 2, 1)
        ...
        x = self.norm(x.permute(0, 2, 1) + icudat)
        return x
    ...
    def forward(self, src):
        ...
        # merge
        mg = torch.cat([item.unsqueeze(1) for item in multi], dim=1)
