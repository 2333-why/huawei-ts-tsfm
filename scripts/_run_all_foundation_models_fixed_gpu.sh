...
# Shared implementation for the fixed two- and eight-GPU public entrypoints.
if (( $# < 2 )); then
    echo "internal runner requires a fixed GPU count and default GPU list" >&2
    exit 2
fi
GPU_COUNT="$1"
DEFAULT_GPUS="$2"
shift 2
case "$GPU_COUNT:$DEFAULT_GPUS" in
    "2:0 1")
        GPU_COUNT_WORD="two"
        ;;
    "8:0 1 2 3 4 5 6 7")
        GPU_COUNT_WORD="eight"
        ;;
    *)
        echo "internal runner supports only the fixed two- and eight-GPU entrypoints" >&2
        exit 2
        ;;
esac
...
declare -A GPU_IDS_SEEN=()
for gpu_id in "${GPU_IDS[@]}"; do
    if ! [[ "$gpu_id" =~ ^(0|[1-9][0-9]*)$ ]]; then
        echo "$GPU_ERROR" >&2
        exit 2
    fi
    if [[ -n "${GPU_IDS_SEEN[$gpu_id]+present}" ]]; then
        echo "$GPU_ERROR" >&2
        exit 2
    fi
    GPU_IDS_SEEN["$gpu_id"]=1
done
